...
import { seedDemo, type AccessContext, type BidPlatform } from '@bid/core';
import type { AppConfig } from './config.js';

/**
 * Composition root for the API process.
 *
 * The API holds exactly one platform instance. Swapping the in-memory store for
 * a Prisma-backed one is a change here and nowhere else — no route, service or
 * domain rule references persistence directly.
 */
export class ApiContext {
  ...
  static async create(config: AppConfig): Promise<ApiContext> {
    ...
    return new ApiContext(platform, config);
  }
}
